...
<div id="breadcrumb" class="container">
    <div class="breadcrumb">
            <a class="home" href="{if isset($force_ssl) && $force_ssl}{$base_dir_ssl}{else}{$base_dir}{/if}" title="{l s='Return to Home'}">Accueil {if isset($path) AND $path}&nbsp;&nbsp;>&nbsp;&nbsp;{/if}</a>      

            {if isset($path) AND $path}

                    {if $path|strpos:'span' !== false}
                            <span class="current_path navigation_page">{$path|@replace:'<a ': '<span itemscope itemtype="http://data-vocabulary.org/Breadcrumb"><a itemprop="url" '|@replace:'data-gg="">': '><span itemprop="title">'|@replace:'</a>': '</span></a></span>'}</span>
                    {else}
                        <span class="current_path">{$path}</span>
                    {/if}
            {/if}
    </div>
</div>
