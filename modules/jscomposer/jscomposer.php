...
//        self::$sds_action_hooks['wpb_save_css_values'] =  array($this,'updateCSSValues');
    ...
    private function checkUpdate(){
        ...
        if(in_array(Tools::getValue('controller'), array('AdminJsComposerSetting', 'AdminModules')) && Tools::getValue('token')){
            ...
            $latest_version = Configuration::get('jscomposer_new_version', $this->version);
            //$timeout = 0;
            $now = time();
            //checking if update is available------------------
            if($now > (int)$timeout) { // set a timeout condition
                    $timelimit = 30 * 60;
                    $jsComposerObject = $this;
                    $jscomposer_key = 'empty'; // just to take update info..
                    $curl = curl_init();
            ...
            if(Tools::version_compare($latest_version, $installed_version, '>')){
                    $this->warning = sprintf($this->l('New version %s is available now! Please update now. Installed version is %s.'), $latest_version, $installed_version);
            }
        }
    }
    ...
    public function validate_module(){ 
        if(in_array(Tools::getValue('controller'), array('AdminJsComposerSetting', 'AdminModules')) && Tools::getValue('token')){
            $jsComposerObject = $this;
            $jscomposer_key = Configuration::get('jscomposer_purchase_key');
            $jscomposer_status = Configuration::get('jscomposer_status', '0');
            $jscomposer_timeout = Configuration::get('jscomposer_update_timeout');
            $timenumber = time();
            $nexttime   = 30 * 60;
            // escape checking for localhost.
            if(in_array($_SERVER['HTTP_HOST'], array('localhost', '127.0.0.1'))){
                return false;
            }
            if($jscomposer_status == '1' || empty($jscomposer_timeout) || (int)$jscomposer_timeout < $timenumber){
                ...
                curl_close($curl);
                $timenumber += $nexttime;
                Configuration::updateValue('jscomposer_update_timeout', $timenumber);
    ...
    {
        if (!parent::install() || !$this->registerHook('displayBackOfficeHeader') || !$this->registerHook('displayBackOfficeFooter') || !$this->registerHook('displayBackOfficeTop') || !$this->registerHook('vcBeforeInit') || !$this->registerHook('VcShortcodesCssClass') || !$this->registerHook('displayHeader') || !$this->registerHook('displayHome') || !$this->registerHook('displayBanner') || !$this->registerHook('displayFooter') || !$this->registerHook('displayTop') || !$this->registerHook('displayTopColumn') || !$this->registerHook('displayProductTab') || !$this->registerHook('displayProductTabContent') || !$this->registerHook('displayLeftColumnProduct') || !$this->registerHook('displayLeftColumn') || !$this->registerHook('displayRightColumnProduct') || !$this->registerHook('displayRightColumn') || !$this->registerHook('displayProductContent') || !$this->registerHook('displaySmartBlogLeft') || !$this->registerHook('displaySidearea') || !$this->registerHook('displayFooterTop') || !$this->registerHook('displaySmartBlogRight') || !$this->registerHook('displaynav') || !$this->registerHook('displayFooterProduct') || !$this->registerHook('displayMaintenance') || !$this->registerHook('displayMyAccountBlock') || !$this->registerHook('displayMyAccountBlockfooter') || !$this->registerHook('actionObjectvccontentanywhereAddAfter') || !$this->registerHook('actionObjectvccontentanywhereUpdateAfter') || !$this->registerHook('actionObjectvccontentanywhereDeleteAfter') || !$this->registerHook('actionObjectvcproducttabcreatorAddAfter') || !$this->registerHook('actionObjectvcproducttabcreatorUpdateAfter') || !$this->registerHook('actionObjectvcproducttabcreatorDeleteAfter') || !$this->registerHook('actionAdminPerformanceControllerAfter') || !$this->registerHook('actionvccontentanywhereUpdate') || !$this->registerHook('actionvcproducttabcreatorUpdate') 
            || !$this->registerHook('VcAllowedImgAttrs')
            || !$this->insertTable() || !$this->installTpls() || !$this->moduleControllerRegistration()
        ){
            return false;
        }
    ...
    {
        if (!parent::uninstall() || !$this->dropTable() || !$this->UninstallTpls() || !$this->moduleControllerUnRegistration()){
            return false;
        }
        return true;
    }
...
//            Media::addJsDef(array('attribute_anchor_separator' => Configuration::get('PS_ATTRIBUTE_ANCHOR_SEPARATOR')));
//        }
...
//                    $this->context->controller->addJqueryUI($js);
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $content = '';
        $this->ajaxController = '//' . Tools::getShopDomain(false) . __PS_BASE_URI__ . $this->context->controller->admin_webpath . '/' . $this->context->link->getAdminLink('VC_ajax');
        if (self::condition()) {
            $this->init();
