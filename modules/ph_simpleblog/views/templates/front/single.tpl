...
<div id="call_to_projet">
    <div id="call_to_projet_content">
        <h1 class="page-heading product-listing" itemprop="headline"> {$post->title|escape:'html':'UTF-8'} </h1>
    </div>
</div>
...
{*			{if Configuration::get('PH_BLOG_DISPLAY_CATEGORY')}
				<span class="post-category">
					<span class="">{l s='Posted in' mod='ph_simpleblog'}:</span> <a href="{$link->getModuleLink('ph_simpleblog', 'category', ['sb_category' => $post->category_rewrite])|escape:'html':'UTF-8'}" title="{$post->category|escape:'html':'UTF-8'}">{$post->category|escape:'html':'UTF-8'}</a>
				</span>
			{/if}*}
		...
		<div class="post-content rte" itemprop="text">
                    ...
                    {if Configuration::get('PH_BLOG_DISPLAY_DATE')}
                        <span class="post-date">
                            <span class="">Date d'ajout :</span> <time itemprop="datePublished" datetime="{$post->date_add|date_format:'c'}">{$post->date_add|date_format:Configuration::get('PH_BLOG_DATEFORMAT')}</time>
                        </span>
                    {/if}
		</div><!-- .post-content -->
